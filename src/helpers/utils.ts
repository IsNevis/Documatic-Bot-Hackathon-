...
export function simpleButtons(
	...
	const button = new MessageButton().setCustomId(customId).setLabel(label).setStyle(style)
...
export function simpleURLButton(url: string, label: string, emoji?: EmojiIdentifierResolvable, disabled?: boolean) {
	const button = new MessageButton().setURL(url).setLabel(label).setStyle('LINK')
...
export function simpleCollector(interaction: CommandInteraction) {
	...
	const filter = (buttonInteraction: Interaction) => {
		return interaction.member?.user.id === buttonInteraction.user.id
	}
