...
export async function invalidButtonEmbed(int: MessageComponentInteraction) {
	const invalidButtonEmbed = simpleEmbed('Invalid Interaction!', `${int.customId} is an invalid Button Interaction!`).setColor(
		'GOLD'
	)
...
export async function batBallTossEmbed(
	int: MessageComponentInteraction,
	totalNumber: number,
	didPlayerGuessCorrectToss: boolean,
	botBatBallTossWord: string
) {
	...
	if (didPlayerGuessCorrectToss) {
		...
		batBallTossEmbed.addField(
			'\u200b',
			`Aw! It seems like the bot wants to **${botBatBallTossWord}**! So you get the other one!`
		)
...
export async function botPlayerBatGameEmbed(
	int: MessageComponentInteraction,
	totalPlayerScore: number,
	wicketsLeft: number,
	totalWickets: number,
	gamePlayerScore: number,
	gameBotScore: number
) {
...
export async function botPlayerBallGameEmbed(
	int: MessageComponentInteraction,
...
export async function botGameWicketsOverEmbed(
	int: MessageComponentInteraction,
	totalPlayerScore: number,
	totalBotScore: number,
	totalWickets: number,
	didPlayerBat: boolean,
	didPlayerBall: boolean,
	gamePlayerScore: number,
	gameBotScore: number
) {
...
export async function botGameOverEmbed(
	int: MessageComponentInteraction,
	totalPlayerScore: number,
	totalBotScore: number,
	totalWickets: number
